...
app.get("/other-api/products", (req, res) => {
  ...
  const idSanPhamGiamGia = req.query.idSanPhamGiamGia || "";
  const giaSauGiam = req.query.giaSauGiam || "";
  ...
  let query = `
      SELECT
          ...
          sp.ten AS sanPham,
          spct.gia AS gia,
          COALESCE(spgg.gia_sau_giam, spct.gia) AS giaSauGiam,
      ...
      WHERE 
          ...
          (COALESCE(spgg.gia_sau_giam, spct.gia) <= ?) AND
          ...
          (spct.deleted = 0) AND (spct.so_luong > 0)
    `;

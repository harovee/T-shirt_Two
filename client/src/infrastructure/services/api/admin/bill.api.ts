...
import {
  API_ADMIN_BILL,
  API_ADMIN_BILL_REFUND,
  API_ADMIN_COUNT_BILL,
} from "@/infrastructure/constants/url";
import {
  DefaultResponse,
  PaginationParams,
  PaginationResponse,
  ResponseList,
} from "@/infrastructure/types/api.common";
import { Ref } from "vue";
...
export const getBillsWait = async () => {
  const res = (await request({
    ...
    method: "GET",
  })) as AxiosResponse<DefaultResponse<Array<BillWaitResponse>>>;
...
export const getBillById = async (billId: string | null) => {
  return (await request({
    ...
    method: "GET",
  })) as AxiosResponse<DefaultResponse<BillResponse>>;
};
...
export const getBillRefundByMaHD = async (billCode: string) => {
  const res = (await request({
    ...
    method: "GET",
  })) as AxiosResponse<DefaultResponse<BillRefundResponse>>;
  return res.data;
};
...
export const updateBillConfirm = async (
  idBill: string,
  params: BillConfirmRequest
) => {
  return (await request({
    url: `${API_ADMIN_BILL}/bill-confirm/${idBill}`,
    method: "PUT",
    data: params,
  })) as AxiosResponse<DefaultResponse<DefaultResponse<null>>>;
};

export const updateBillWait = async (
  idBill: string,
  params: BillWaitRequest
) => {
  return (await request({
...
export const changeBillStatus = async (
  idBill: string,
  params: ChangeStatusBillRequest
) => {
  return (await request({

<template>
          ...
          <a-image-preview-group>
              ...
              <div v-if="column.dataIndex === 'chiTiet'" class="text-left">
                <a-space direction="vertical">
                  ...
                  <a-space>
                    ...
                    <div :style="{width:'25px', height: '25px', background: record.maMauSac, 'border-radius': '5px' }" class=""></div>
                    {{ record.maMauSac  }}
                  </a-space>
                </a-space>
              </div>
              ...
              <div v-if="column.dataIndex === 'giaBanHienTai'" class="text-right">
                <a-typography-text type="danger" strong underline class="cursor-pointer">
                    {{ formatCurrency(record.giaHienTai ? record.giaHienTai : record.gia, 'VND', 'vi-VN')  }}
                </a-typography-text>
              </div>
              <div v-if="column.dataIndex === 'thanhTien'" class="text-right">
                <a-typography-text type="danger" strong underline class="cursor-pointer">
                    {{ formatCurrency(record.giaHienTai ? record.giaHienTai*record.soLuong : record.gia*record.soLuong, 'VND', 'vi-VN')  }}
                </a-typography-text>
              </div>
              <div v-else-if="column.dataIndex === 'hanhDong'" class="flex items-center justify-center space-x-2">
                <a-popconfirm
  ...
  import { POSProductDetailResponse } from "@/infrastructure/services/api/admin/point-of-sale.api";
  import { useGetOrderDetails} from "@/infrastructure/services/service/admin/point-of-sale";
...
//   
  ...
  const { data, isLoading, refetch } = useGetOrderDetails(props.idOrder?.valueOf(), {

    ...
    <div class="flex justify-between items-center">
      <div>
        ...
        <p class="text-sm text-gray-500">
          Hiển thị danh sách danh mục giá T-Shirts Two
        </p>
      </div>
      <div class="p-2.5">
        <a-tooltip title="Thêm danh mục" trigger="hover">
          <a-button
            class="bg-purple-300 flex justify-between items-center gap-2"
            size="large"
            @click="$emit('handleOpenModalCreateCategory')"
          >
        ...
        >
          <a-tooltip title="Sửa" trigger="hover">
            <a-button
              ...
              size="middle"
              shape="round"
              @click="$emit('handleOpenModalUpdateCategory', record)"
            >
...
const redirectToCreateCategory = () => {
  router.push(
    ROUTES_CONSTANTS.ADMIN.children.PRODUCTS.children.CATEGORY.path
  );
};

const handleRedirect = (id) => {
  router.push(
    `${ROUTES_CONSTANTS.ADMIN.children.PRODUCTS.children.CATEGORY.path}/${id}`
  );
};

const columnsCategory: ColumnType[] = [
  {
    title: "STT",
    dataIndex: "catalog",
    key: "catalog",

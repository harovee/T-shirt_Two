<template>
  ...
  <div class="p-6 grid grid-cols-5 gap-6">
    <div
      class="col-span-2 md:col-span-5 lg:col-span-2 bg-gray-100 rounded-xl p-5 h-fit"
    >
      <div class="flex justify-end">
        <a-tag v-if="detailRef.status === false" color="success"
          >Hoạt động</a-tag
        >
        <a-tag v-else-if="detailRef.status === true" color="warning"
          >Vô hiệu hóa</a-tag
        >
      ...
      <div class="mt-5 p-5 w-full h-full bg-white rounded-xl">
        <h4 class="text-center text-xl font-semibold mb-5 text-gray-700">
          {{ detailRef.fullName }}
        </h4>
        <p class="text-gray-500">Mã khách hàng: KH{{ detailRef.code }}</p>
        <p class="text-gray-500">
          Lịch sử tạo: {{ detailRef.createdBy || "Chưa xác định" }} lúc
          {{ convertDateFormat(detailRef.createdDate) }}
        </p>
        <p class="text-gray-500">
          Chỉnh sửa lần cuối:
          {{ detailRef.lastModifiedBy || "Chưa xác định" }} lúc
          {{ convertDateFormat(detailRef.lastModifiedDate) }}
        </p>
      </div>
    </div>
...
import {
  convertDateFormat,
  convertToAntdDatePicker,
} from "@/utils/common.helper";
import { ExclamationCircleOutlined } from "@ant-design/icons-vue";
import ClientAddress from "@/page/admin/client/ClientAddress.vue";
import dayjs from "dayjs";
import ClientAddressCreateModal from "./ClientAddressCreateModal.vue";
import { useAuthStore } from "@/infrastructure/stores/auth";
import {CLOUDINARY_CLOUD_NAME, CLOUDINARY_UPLOAD_PRESET} from "@/infrastructure/constants/cloudinary";
...
/*** GetClientAddress  ***/
const { data: clientAddressesData } = useGetClientAddressesByClientId(
  clientId.value,
  {
    refetchOnWindowFocus: false,
    placeholderData: keepPreviousData,
...
function getShortHeader(clientAddress) {
  const prefix = clientAddress.isDefault
    ? "Địa chỉ mặc định: "
    : "Địa chỉ thường: ";
  const maxLength = 30;
  return (
    prefix +
    (clientAddress.line.length > maxLength
      ? clientAddress.line.substring(0, maxLength) + "..."
      : clientAddress.line)
  );
}
